...
@test "push private package liget-two v1.0.0" {
  run /bin/bash -c "cd ../input/liget-two/bin/Debug/ && dotnet nuget push liget-two.1.0.0.nupkg --source http://liget:9011/v3/index.json --api-key NUGET-SERVER-API-KEY"
  assert_output --partial "Your package was pushed"
  assert_equal "$status" 0
}

@test "nuget install latest package version (1.0.0) via V3" {
  run /bin/bash -c "cd nuget && nuget install liget-two -DisableParallelProcessing -NoCache -DirectDownload -Source http://liget:9011/v3/index.json"
...
@test "push private package liget-two v2.1.0" {
  run /bin/bash -c "cd ../input/liget-two/bin/Debug/ && dotnet nuget push liget-two.2.1.0.nupkg --source http://liget:9011/v3/index.json --api-key NUGET-SERVER-API-KEY"
  assert_output --partial "Your package was pushed"
  assert_equal "$status" 0
}

@test "check registration endpoint includes v2.1.0" {
  run /bin/bash -c "curl http://liget:9011/v3/registration/liget-two/index.json"
  assert_output --partial "2.1.0"
  assert_equal "$status" 0
}

@test "nuget install latest package version (2.1.0) via V3" {
  run /bin/bash -c "cd nuget && nuget locals http-cache -clear && nuget install liget-two -DisableParallelProcessing -NoCache -DirectDownload -Source http://liget:9011/v3/index.json"
  refute_output --partial 'Could not download'
  refute_output --partial 'went wrong'
