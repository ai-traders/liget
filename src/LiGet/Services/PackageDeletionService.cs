...
namespace LiGet.Services
{
    public class PackageDeletionService : IPackageDeletionService
    {
        ...
        private readonly LiGetOptions _options;
        ...
        public async Task<bool> TryDeletePackageAsync(PackageIdentity pid)
        {
            switch (_options.PackageDeletionBehavior)
            {
                case PackageDeletionBehavior.Unlist:
                    return await TryUnlistPackageAsync(pid);

                case PackageDeletionBehavior.HardDelete:
                    return await TryHardDeletePackageAsync(pid);
        ...
        private async Task<bool> TryHardDeletePackageAsync(PackageIdentity pid)
        {
            string id = pid.Id;
            NuGetVersion version = pid.Version;
            ...
            var found = await _packages.HardDeletePackageAsync(pid);
            if (!found)
            {
                _logger.LogWarning(
            ...
            await _storage.DeleteAsync(pid);
