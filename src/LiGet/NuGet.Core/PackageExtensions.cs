...
namespace NuGet
{
    public static class PackageExtensions
    {
        ...
        {
            return packageMetadata.Version.ReleaseLabels == null;
        }
        ...
        public static bool IsSatellitePackage(this IPackageMetadata package)
        {
                    ...
                    package.Id.EndsWith('.' + package.Language, StringComparison.OrdinalIgnoreCase))
            {
                ...
                return package.DependencySets.SelectMany(s => s.Packages).Any(
                       d => d.Id.Equals(corePackageId, StringComparison.OrdinalIgnoreCase) &&
                       d.VersionRange != null &&
                       d.VersionRange.MaxVersion == d.VersionRange.MinVersion && d.VersionRange.IsMaxInclusive && d.VersionRange.IsMinInclusive);
            }
            return false;
        }
        ...
        public static IEnumerable<ServerPackage> FilterByPrerelease(this IEnumerable<ServerPackage> packages, bool allowPrerelease)
        {
            if (packages == null)
            {
                return null;
        ...
        public static IQueryable<T> Find<T>(this IQueryable<T> packages, IEnumerable<string> propertiesToSearch, string searchText) where T : IPackage
        {
            ...
            {
                throw new ArgumentException("Argument cannot be null", "propertiesToSearch");
            }
